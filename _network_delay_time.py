import collections
def findClosest(best, visited):
        highestVal = float('inf')
        highestKey = None
        for k, v in best.items():
            if k not in visited and (not highestKey or v < highestVal):
                highestKey = k
        return highestKey
...
#       a: 2, b: 3
    ...
    for _ in range(N): 
        closest = findClosest(best, visited)
        visited.add(closest)
        for neighbor in graph[closest]:
#               src to curr + curr to neigh
            best[neighbor] = min(best[closest] + graph[closest][neighbor], best[neighbor])
    print(best)
    print(visited)
